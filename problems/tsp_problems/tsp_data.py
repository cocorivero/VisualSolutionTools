...
def solutions_adapter(manager, routing, assignment):
    index = routing.Start(0)
    nodes_visited = [manager.IndexToNode(index)]
    route_distance = 0
    while not routing.IsEnd(index):
        previous_index = index
        index = assignment.Value(routing.NextVar(index))
...
def main_tsp():
    ...
    if assignment:
        return solutions_adapter(manager, routing, assignment)
        # print(solutions_adapter(manager, routing, assignment))
    # [END print_solution]
